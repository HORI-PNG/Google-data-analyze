// データラベル（棒グラフの上の数字）の有効化と、アニメーションの無効化
Chart.register(ChartDataLabels);
Chart.defaults.animation = false;

// グラフのインスタンス（実体）を保存する変数。再描画時に古いグラフを破棄するために使う
let charts = {
...
function switchTab(btnElement, targetId) {
    const panel = btnElement.closest('.panel'); // 所属するパネル(AかB)を取得
    ...
    // クリックされたタブと対象コンテンツをアクティブにする
    document.getElementById(targetId).classList.add('active');
    btnElement.classList.add('active');
}

// オブジェクト({A: 10, B: 5})を文字列("A(10人) / B(5人)")に変換する補助関数
...
function drawExplChart(side, data, dateStr) {
    ...
    if (charts[side].expl) charts[side].expl.destroy(); // 古いグラフを破棄

    // ★ 9通り対応: 色の設定（一人暮らしは濃く、実家は普通、迷いは薄くする）
    charts[side].expl = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                ...
                datalabels: {
                    color: '#444', anchor: 'end', align: 'top', font: { size: 9 },
                    formatter: value => value > 0 ? value : '' // 0人の場合は数値を表示しない
                }
            }
        }
    });
...
function drawExplPctChart(side, data, dateStr) {
    ...
    charts[side].explPct = new Chart(ctx, {
        ...
        options: {
            ...
            scales: {
                x: { ticks: { autoSkip: false, maxRotation: 45, minRotation: 45, font: { size: 11 } } },
                y: { beginAtZero: true, max: 115 } // 100%の上部にデータラベル用の隙間を作る
            },
            plugins: {
                legend: { position: 'top', labels: { padding: 20 } }, // 凡例の下に余白を作る
